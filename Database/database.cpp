...
bool DataBase::verifglobalvariablesSQL()
{
    ...
    bool ok;
    QVariantList vardata = getFirstRecordFromStandardSelectSQL("SHOW VARIABLES LIKE \"sql_mode\";", ok);
    if (ok && vardata.size()>0)
        result =  vardata.at(1).toString();
    return (result == sqlmode);
}

bool DataBase::dirsecure_file_priv()
{
...
void DataBase::initParametresSysteme()
{
    ...
    //! from versionbase 82 récupération de la version linguistique enregistrée en base (parce que la lecture de cette valeur intervient avant le contrôle de la version en base)
    req = "SELECT COUNT(*) FROM "
          "(SELECT COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS "
...
void DataBase::setversionbaseiol(double version)
{
    ...
    parametres()->setversionbaseiol(version);
}
 ...
 * l'ancienne base ne sont pas fiables). Le type se lit à la forme du code :
 * - présent dans la table officielle ccam                       -> CCAM (1)        -> jointuresccam ;
 ...
 * des autres utilisateurs serait orphelin après dédup (loadUserCotations retrouve la cotation par
 * son idcotation).
 */
void DataBase::exporteJointures()
{
    ...
    //! filtre « autre » : ni NGAP (AMY), ni code CCAM isolé, ni association
    const QString autre  = " not like 'AMY%' and " CP_TYPEACTE_COTATIONS " not" + inCCAM
 ...
 * dernière étape de la migration : la classification et l'export vers les jointures ayant été faits
 * par exporteJointures, on ne garde plus qu'UNE ligne par Typeacte dans cotations (la référence, le
 * plus petit idcotation) et on supprime les doublons laissés par le partage d'une cotation entre
 * plusieurs utilisateurs. Les montants pratiqués vivent désormais dans les tables de jointures,
 * tous rattachés à cette référence.
 */
void DataBase::nettoieTableCotations()
{
    if (!m_db.isOpen())
        return;
    //! une seule ligne par Typeacte (on garde le plus petit idcotation, cible des jointures)
...
{
    QList<PosteConnecte*> postes = QList<PosteConnecte*>();
...
{
    QList<Correspondant*> correspondants = QList<Correspondant*>();
...
{
    QList<Impression*> impressions = QList<Impression*>();
...
{
    QList<Depense*> depenses = QList<Depense*>();
...
{
    QList<Archive*> archives = QList<Archive*>();
...
{
    QList<Banque*> banques = QList<Banque*>();
...
{
    QList<Tiers*> listetiers = QList<Tiers*>();
...
{
    QList<Recette*> listerecettes = QList<Recette*>();
...
QList<Cotation*> DataBase::loadCotationsByUser(User *usr)
{
    ...
    requetes << "select cot." CP_ID_COTATIONS ", cot." CP_TYPEACTE_COTATIONS ", cot." CP_MONTANTOPTAM_COTATIONS ", cot." CP_MONTANTNONOPTAM_COTATIONS ", "
                ...
                " from " TBL_COTATIONS " cot"
                " inner join " TBL_JOINTURESASSOCIATIONS " jt on jt." CP_IDCOTATION_JOINTASSOCIATIONS " = cot." CP_ID_COTATIONS
    ...
    requetes << "select cot." CP_ID_COTATIONS ", cot." CP_TYPEACTE_COTATIONS ", cot." CP_MONTANTOPTAM_COTATIONS ", cot." CP_MONTANTNONOPTAM_COTATIONS ", "
                ...
                " from " TBL_COTATIONS " cot"
                " inner join " TBL_JOINTURESNGAP " jt on jt." CP_IDCOTATION_JOINTNGAP " = cot." CP_ID_COTATIONS
    ...
    requetes << "select cot." CP_ID_COTATIONS ", cot." CP_TYPEACTE_COTATIONS ", jt." CP_MONTANTCONVENTIONNEL_JOINTAUTRESCOTATIONS ", jt." CP_MONTANTCONVENTIONNEL_JOINTAUTRESCOTATIONS ", "
                       "jt." CP_MONTANTPRATIQUE_JOINTCOTATIONS ", cot." CP_TYPECOTATION_COTATIONS ", cot." CP_FREQUENCE_COTATIONS ", cot." CP_TIP_COTATIONS
                " from " TBL_COTATIONS " cot"
                " inner join " TBL_JOINTURESAUTRESCOTATIONS " jt on jt." CP_IDCOTATION_JOINTAUTRESCOTATIONS " = cot." CP_ID_COTATIONS
                " where jt." CP_IDUSER_JOINTAUTRESCOTATIONS " = " + id +
                " order by cot." CP_TYPEACTE_COTATIONS;

    for (int r = 0; r < requetes.size(); ++r)
    {
        QList<QVariantList> cotlist = StandardSelectSQL(requetes.at(r), ok);
        if (!ok)
            continue;
...
{
    QList<Motif*> motifs = QList<Motif*>();
...
{
    QList<Ville*> villes = QList<Ville*>();
...
{
    QList<PatientEnCours*> listpat = QList<PatientEnCours*>();
    QString req = "SELECT " CP_IDPAT_SALDAT ", " CP_IDUSERSUPERVISEUR_SALDAT ", " CP_STATUT_SALDAT ", " CP_HEURESTATUT_SALDAT ", " CP_HEURERDV_SALDAT ", "
                  CP_HEUREARRIVEE_SALDAT ", " CP_MOTIF_SALDAT ", " CP_MESSAGE_SALDAT ", " CP_IDACTEAPAYER_SALDAT ", " CP_POSTEEXAMEN_SALDAT ", "
...
QList<Refraction*> DataBase::loadRefractionsByPatId(int id)                  //! charge toutes les refractions d'un patient
{
    ...
    QString req = "SELECT " CP_ID_REFRACTIONS ", " CP_IDPAT_REFRACTIONS ", " CP_IDACTE_REFRACTIONS ", " CP_DATE_REFRACTIONS ", " CP_TYPEMESURE_REFRACTIONS ", "          // 0-1-2-3-4
        ...
        CP_RYSEROG_REFRACTIONS ", " CP_FORMULEOG_REFRACTIONS ", " CP_COMMENTAIREORDO_REFRACTIONS ", " CP_TYPEVERRES_REFRACTIONS ", " CP_OEIL_REFRACTIONS ", "            // 35-36-37-38-39
    ...
    req += " WHERE " CP_IDPAT_REFRACTIONS " = " + QString::number(id) ;
    req += " order by " CP_ID_REFRACTIONS " desc";
...
{
    QList<QPair<int, int>> listpair = QList<QPair<int, int>>();
...
QList<IOL*> DataBase::loadIOLs()                                            //! charge tous les IOLS
{
    ...
    QString req =   "SELECT " CP_ID_IOLS ", " CP_IDMANUFACTURER_IOLS ", " CP_IDIOLCON_IOLS ", " CP_MODELNAME_IOLS ", " CP_DIAOPT_IOLS ", "          // 0-1-2-3-4
                    ...
                    CP_HOLL1_IOLS ", " CP_HOFFERQ_IOLS ", " CP_BARRETTLF_IOLS ", " CP_BARRETTDF_IOLS ", " CP_OLSEN_IOLS ", "                        // 20-21-22-23-24
...
QList<RefractionDevice*> DataBase::loadrefractionDevices()
{
    QList<RefractionDevice*> refractiondevices = QList<RefractionDevice*>();
    QString req = "select " CP_ID_REF ", " CP_BRAND_REF ", " CP_MODEL_REF ", " CP_TYPE_REF ", " CP_LIKEDEVICE_REF
                  ", " CP_SERIAL_REF ", " CP_LAN_REF ", " CP_IMPLEMENTE_REF ", " CP_COMMENT_REF
                  " from " TBL_APPAREILSREFRACTION " ORDER BY " CP_BRAND_REF ", " CP_MODEL_REF;
    QList<QVariantList> devlist = StandardSelectSQL(req, ok);
    if(!ok || devlist.size()==0)
        return refractiondevices;
    for (int i=0; i<devlist.size(); ++i)
    {
        QJsonObject jData{};
        jData[CP_ID_REF]    = devlist.at(i).at(0).toInt();
        jData[CP_BRAND_REF]        = devlist.at(i).at(1).toString();
        jData[CP_MODEL_REF]        = devlist.at(i).at(2).toString();
        jData[CP_TYPE_REF]          = devlist.at(i).at(3).toString();
        jData[CP_LIKEDEVICE_REF]    = devlist.at(i).at(4).toInt();
        jData[CP_SERIAL_REF]        = (devlist.at(i).at(5).toInt() == 1);
        jData[CP_LAN_REF]           = (devlist.at(i).at(6).toInt() == 1);
