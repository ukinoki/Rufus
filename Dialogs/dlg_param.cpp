    ...
    ui(new Ui::dlg_param)
{
    ...
    for (int l=0; l< listportsbox.size(); ++l) {
        m_connectionsPortsComboBox.append(
            connect(listportsbox.at(l),   &QComboBox::currentTextChanged,    this,   [=, this] (QString s) {
                if (s!= listportsbox.at(l)->valeuravant())
                {
                    ...
                    if (QSerialPortInfo::availablePorts().size()>0)
                        proc->Ouverture_Ports_Series(nullptr);
                    proc->Ouverture_Fichiers_Echange(nullptr);
                }
            })
        );
    }
   ...
   /*-------------------- GESTION DES VILLES ET DES CODES POSTAUX-------------------------------------------------------*/
    ...
    QListWidgetItem *rowidg = new QListWidgetItem (QPixmap("://Romania.ico"), "Română");
    rowidg                  ->setData(Qt::UserRole, "RO");
    contents                ->addItem(rowidg);
    QListWidgetItem *cawidg = new QListWidgetItem (QPixmap("://Catalonia.ico"), "Català");
    cawidg                  ->setData(Qt::UserRole, "CA");
    contents                ->addItem(cawidg);
    QList<QListWidgetItem*> listwidg = QList<QListWidgetItem*>() << enwidg << frwidg << eswidg << brwidg << ptwidg << itwidg << rowidg << cawidg;
...
dlg_param::~dlg_param()
{
    for (const auto& conn : m_connectionsPortsComboBox)
        disconnect(conn);
    delete ui;
}
...
void dlg_param::MAJCotation(QStandardItem *itcheck)
{
    ...
    const int iduser     = currentuser()->id();
    ...
    //! --- cochage : (re)crée la jointure du user avec le pratiqué par défaut ---
    if (cot->isNGAP())                              //! NGAP : jointure sans montant, pas d'édition du pratiqué
    {
        db->ajouteJointureCotation(typ, cot->id(), iduser, 0);
        return;
    }
...
void dlg_param::supprimeCotation(Cotation *cot)
{
    ...
    //! plus personne ne l'utilise (jointure du user retirée -> « par un autre » = « par quiconque »)
    //! -> on supprime la cotation elle-même, sauf NGAP (stockée là uniquement, on la garde)
    if (!cot->isNGAP() && !cotationUtiliseeParAutre(cot))
        db->SupprRecordFromTable(cot->id(), CP_ID_COTATIONS, TBL_COTATIONS);
    Remplir_TableCotations();
    enableCotations();
 ...
 *  clés SSL recopiées pour le seul accès distant.
 *  \param mode  Utils::ReseauLocal ou Utils::Distant */
void dlg_param::ImporterDonneesConnexion(Utils::ModeAcces mode)
{
    ...
    if (!QFile::exists(source + "/" + FIC_ADRESSE) || (distant && !QDir(source + "/" + DIR_CLES).exists()))
    {
    ...
    const QString adresse = connexion.value(distant? CLE_SERVEUR : CLE_LOCAL).toString();
    ...
    {
        ui->EmplacementLocaluplineEdit  ->setText(adresse);
        ui->SQLPortLocalcomboBox        ->setCurrentText(port);
        ui->MDPLocaluplineEdit          ->setText(mdp);
    }
