    ...
    ui(new Ui::dlg_bilanortho)
{
    ...
    restoreGeometry(proc->settings()->value(Position_Fiche Nom_fiche_BilanOrtho).toByteArray());
...
void dlg_bilanortho::ImprimeBOClicked()
{
    ...
    textentete = proc->CalcEnteteImpression(l_currentdate, userEntete, false).value(NORMHeader);
...
{
    proc->settings()->setValue(Position_Fiche Nom_fiche_BilanOrtho, saveGeometry());
    event->accept();
}
...
QString dlg_bilanortho::calcReponsehTml(QString textorigin)
{
    ...
    QStringList listrep = Reponse.split(QRegularExpression::wildcardToRegularExpression("<td width=\"[0-9]{1,3}\">"));
    if (listrep.size()>1)
