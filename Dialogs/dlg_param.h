...
private:
    ...
    void                ImporterDonneesConnexion(Utils::ModeAcces mode);  //!< (réseau local ou accès distant) réinjecte sur ce poste les données de connexion exportées depuis le serveur
    QString             AdresseIPPublique();             //!< adresse publique vue de l'extérieur, vide si le service ne répond pas
    void                ExporterDonneesConnexionDistant();  //!< (accès distant) exporte vers une clé USB les clés, l'adresse, le port et le mot de passe de ce serveur distant, pour paramétrer un autre poste
