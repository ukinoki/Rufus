    ...
    ui(new Ui::dlg_paiementdirect)
{
    ...
    ui->ComptablescomboBox->setEnabled(Datas::I()->users->getById(currentuser()->idcomptableactes()) == nullptr && map_comptables->size()>1);
    ...
    if (m_listidactes.size() > 0)                     //! il y a un ou pusieurs actes à enregistrer - l'appel a été fait dans rufus.cpp depuis l'accueil ou par le bouton enregistrepaiement
    {
        Acte *act = Datas::I()->actes->getById(m_listidactes.at(0));
        if (act != nullptr)
            m_useracrediter = Datas::I()->users->getById(act->idComptable());
    }
    else if (map_comptables->size() == 1)                                           //! la fiche a été appelée par le menu et il n'y a pas d'acte prédéterminé à enregistrer
...
void dlg_paiementdirect::AfficheActeVerrouille()
{
    ...
    {
        UpCheckBox* Check = qobject_cast<UpCheckBox*>(ui->SalleDAttenteupTableWidget->cellWidget(i,1));
        if (Check)
...
{
    for (int i= 0; i<ui->ListeupTableWidget->rowCount(); ++i)
        if (ui->ListeupTableWidget->item(i,0) != nullptr)
            ui->ListeupTableWidget->setRowHidden(i, (ui->ListeupTableWidget->item(i,0)->data(Qt::UserRole).toInt() != m_useracrediter->id()) && m_useracrediter != nullptr);
    for (int i= 0; i<ui->SalleDAttenteupTableWidget->rowCount(); ++i)
        if (ui->SalleDAttenteupTableWidget->item(i,0) != nullptr)
            ui->SalleDAttenteupTableWidget->setRowHidden(i, (ui->SalleDAttenteupTableWidget->item(i,0)->data(Qt::UserRole).toInt() != m_useracrediter->id()) && m_useracrediter != nullptr);
...
void dlg_paiementdirect::EnregistreNouveauPaiement()
{
    ...
    bool ok = true;
    RemplitLesTables(ok);
    if (map_comptables->size() == 1 || Datas::I()->users->getById(currentuser()->idcomptableactes()) != nullptr)
    {
        FiltreLesTables();
        if (ui->ListeupTableWidget->rowNoHiddenCount() == 0
            && ui->SalleDAttenteupTableWidget->rowNoHiddenCount() == 0)
        {
            m_mode = Accueil;
            ui->RecImageLabel->setVisible(false);
            RegleAffichageFiche();
            UpMessageBox::Watch(this, tr("Aucun acte à enregistrer"), tr("Il n'y a pas d'acte à enregistrer"));
            m_traiteparcloseflag = false;
        }
        else
        {
    ...
    else  /*! l'utilisateur est un administratif qui peut gérer plusieurs comptes */
    {
        QMap<int, User*> lmap_comptables;
        for (auto it = map_comptables->constBegin(); it != map_comptables->constEnd(); ++it)
        {
            User *usr = const_cast<User*>(it.value());
            if (usr)
                lmap_comptables.insert(usr->id(),usr);
        }
        ChangeComptable(lmap_comptables.cbegin().value());
        while (lmap_comptables.size()>0) {
            if (ui->ListeupTableWidget->rowNoHiddenCount() == 0
                && ui->SalleDAttenteupTableWidget->rowNoHiddenCount() == 0)
            {
                lmap_comptables.remove(m_useracrediter->id());
                if(lmap_comptables.size()==0)      /*! personne n'a d'acte à enregistrer */
                {
                    m_mode = Accueil;
                    ui->RecImageLabel->setVisible(false);
                    RegleAffichageFiche();
                ...
                else /*! le comptable en cours n'a pas d'acte à  enregistrer, on passe à un autre comptable */
                    ChangeComptable(lmap_comptables.cbegin().value());
            }
            else /*! on a trouvé un comptable qui a des actes à enregistrer, on se positionne sur lui et on sort */
            {
                RegleAffichageFiche();
                if (ui->DetailupTableWidget->rowCount() > 0)
                    ui->OKupPushButton->setEnabled(true);
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void dlg_paiementdirect::RegleAffichageFiche(bool filtre)
{
    ...
    ui->ComptablescomboBox          ->setEnabled(Datas::I()->users->getById(currentuser()->idcomptableactes()) == nullptr
    ...
    disconnect (ui->SalleDAttenteupTableWidget, &QTableWidget::itemSelectionChanged, this, nullptr);
...
void dlg_paiementdirect::RenvoieRangee(bool Coche)
{
    ...
    case EnregistrePaiement:
    {
        UpTableWidget* TableOrigine = qobject_cast<UpTableWidget*>(focusWidget());
    ...
    case VoirListeActes:
    {
        UpTableWidget*           TableOrigine = qobject_cast<UpTableWidget*>(sender());
        if (!TableOrigine) return;
...
bool dlg_paiementdirect::eventFilter(QObject *obj, QEvent *event)
{
            ...
            if (mouseEvent->button() == Qt::LeftButton)
            {
                ...
                if (CheckBox->Toggleable())
                {
                        ...
                        && !CheckBox->isChecked())
                    {
                        ...
                        if (!isActeLocked(TableAVerifier->item(CheckBox->rowTable(),0)->text().toInt()))
                            return true;
                    }
    ...
    if (event->type() == QEvent::KeyPress)
    {
                ...
                && keyEvent->modifiers() == Qt::NoModifier)
        {
            ...
            else if (qobject_cast<UpPushButton *>(obj) != nullptr && obj->objectName() != "dlg_paiement")
            {
...
void dlg_paiementdirect::CompleteDetailsTable(UpTableWidget *TableSource, int Rangee, bool Coche)
{
    ...
    case EnregistrePaiement:            // La table est remplie par un clic dans le checkbox de sélection
    {
        ...
        if (Coche)
        {
            if (!PoseVerrouCompta(TableSource->item(Rangee,0)->text().toInt()))
            {
                /*! un autre poste a posé le verrou entre la vérification au clic et maintenant : on décoche et on prévient */
                UpCheckBox* Check = qobject_cast<UpCheckBox*>(TableSource->cellWidget(Rangee,1));
                if (Check)
                    Check->setChecked(false);
                isActeLocked(TableSource->item(Rangee,0)->text().toInt());
                return;
            }
            ...
            pItem1->setText(TableSource->item(Rangee,0)->text());                           // idActe
            pItem1->setData(Qt::UserRole,TableSource->item(Rangee,0)->data(Qt::UserRole));  // UserComptable
            ...
            pItem2->setText(TableSource->item(Rangee,2)->text());                           // Date
            pItem3->setText(TableSource->item(Rangee,3)->text());                           // Nom Prenom
            pItem3->setData(Qt::UserRole,TableSource->item(Rangee,3)->data(Qt::UserRole));  // Superviseur, Comptable, DDN, Nom
            pItem4->setText(TableSource->item(Rangee,4)->text());                           // Cotation
            pItem5->setText(TableSource->item(Rangee,5)->text());                           // Montant
            QString ResteDu = QLocale().toString(QLocale().toDouble(TableSource->item(Rangee,7)->text()),'f',2);
            pItem6->setText(ResteDu);                                                       // Reste dû
            pItem7->setText(TableSource->item(Rangee,8)->text());                           // Classementpardate
        ...
        {
            RetireVerrouCompta(TableSource->item(Rangee,0)->text().toInt());

            QList<QTableWidgetItem*> items;
            items = ui->DetailupTableWidget->findItems(TableSource->item(Rangee,0)->text(),Qt::MatchExactly);
            if (!items.isEmpty())
            {
                ...
                QStringList ListeDates, ListeNomsPrenoms, ListeCotations;
                ...
                for (int k = 0; k < ui->DetailupTableWidget->rowCount();k++)
                {
                    ...
                    ListeNomsPrenoms                << ui->DetailupTableWidget->item(k,3)->text();
                    DatasList                       <<  ui->DetailupTableWidget->item(k,3)->data(Qt::UserRole);
                ...
                for (int l = 0; l < NbLignesAVirer ;l++)
                {
                    ...
                    pItem3->setText(ListeNomsPrenoms.at(l));        //Nom Prenom
                    pItem3->setData(Qt::UserRole, DatasList.at(l)); // Superviseur, Comptable, DDN, Nom
    ...
    case VoirListeActes:                // La table est remplie par la sélection d'une ligne dans listuptablewidget ou salleDAttenteupTablewidget
    {
        ...
        if (TableSource == ui->ListeupTableWidget)
        {
            ...
            RemplirTableWidget(ui->SalleDAttenteupTableWidget,Payment,reclist,false,Qt::Unchecked);
            if (reclist.size() == 0)
            {
        ...
        QString TextidRecette   = TableSource->item(Rangee,0)->text();
        ...
        if (tiersdata.size() > 0)
        {
            ...
            RemplirTableWidget(ui->DetailupTableWidget,Acts,pmtlist,false,Qt::Unchecked);
...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
bool dlg_paiementdirect::PoseVerrouCompta(int ActeAVerrouiller)
{
    ...
    QString verrourequete = "select " CP_POSEPAR_VERROUCOMPTA " from " TBL_VERROUCOMPTAACTES " where " CP_IDACTE_VERROUCOMPTA " = " + QString::number(ActeAVerrouiller);
    QVariantList actdata = db->getFirstRecordFromStandardSelectSQL(verrourequete, m_ok);
    return (m_ok && actdata.size() > 0 && actdata.at(0).toInt() == currentuser()->id());
}
...
void dlg_paiementdirect::RemplitLesTables(bool &ok)
{
    QString             requete;
    disconnect (ui->SalleDAttenteupTableWidget, &QTableWidget::itemSelectionChanged, this, nullptr);
    disconnect (ui->SalleDAttenteupTableWidget, &QTableWidget::itemEntered, this, nullptr);
    disconnect (ui->ListeupTableWidget,         &QTableWidget::itemSelectionChanged, this, nullptr);
    disconnect (ui->ListeupTableWidget,         &QTableWidget::itemEntered, this, nullptr);
    ...
    case EnregistrePaiement:
        ...
        */
        ...
        RemplirTableWidget(ui->SalleDAttenteupTableWidget,Acts, actlist, true, Qt::Unchecked);
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void dlg_paiementdirect::RemplirTableWidget(QTableWidget *TableARemplir, ModeTable modetable, QList<QVariantList> Tablelist, bool AvecUpcheckBox, Qt::CheckState CheckedOuPas)
{
    ...
    if (Tablelist.size() == 0)
        return;
    switch  (modetable) {
    case Acts:                                                                       // Table affichant des actes
        for (int i = 0; i < Tablelist.size(); i++)
            {
                ...
                if (TableARemplir == ui->ListeupTableWidget)
                {
                    ...
                    if (Datas::I()->users->getById(Tablelist.at(i).at(11).toInt()) != nullptr)
                        superviseur = Datas::I()->users->getById(Tablelist.at(i).at(12).toInt())->login();
                    if (Datas::I()->users->getById(Tablelist.at(i).at(11).toInt()) != nullptr)
                        comptable = Datas::I()->users->getById(Tablelist.at(i).at(11).toInt())->login();
                    pItem3->setData(Qt::UserRole, QStringList()
                                    ...
                                    << Tablelist.at(i).at(2).toString());                                 // Superviseur, Comptable, DDN, Nom
                }
                else
                    pItem3->setData(Qt::UserRole, QStringList()
                                    << ""
    ...
    case Payment:                                                                   // Table affichant des paiements
        for (int i = 0; i < Tablelist.size(); i++)
            {
                int col = 0;
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void dlg_paiementdirect::VideDetailsTable(int lign)
{
    bool                    idActeTrouve = false;

    // on récupère l'idActe de l'item concerné
    QString ActeAVirer = ui->DetailupTableWidget->item(lign,0)->text();
    if (m_mode == EnregistrePaiement)
    {
        QList<QTableWidgetItem*> items;
    ...
    for (int k=0; k<cols; k++)
        ui->DetailupTableWidget ->removeCellWidget(lign,k);
    ui->DetailupTableWidget->removeRow(lign);
    TrieListe(ui->DetailupTableWidget);
    CalculTotalDetails();
...
void dlg_paiementdirect::ClearCheqData()
{
    ui->TireurChequelineEdit    ->clear();
    ui->BanqueChequecomboBox    ->clearEditText();
    ui->EnAttentecheckBox       ->setChecked(false);
}
...
void dlg_paiementdirect::RetireVerrouCompta(int ActeADeverrouiller)
{
    ...
    db->StandardSQL("DELETE FROM " TBL_VERROUCOMPTAACTES
...
{
    db->StandardSQL("delete from " TBL_VERROUCOMPTAACTES " where " CP_POSEPAR_VERROUCOMPTA " = " + QString::number(currentuser()->id()) + " or PosePar is null");
}
...
void dlg_paiementdirect::RemetToutAZero()
{
    ...
    VideTable(ui->DetailupTableWidget);
    VideTable(ui->SalleDAttenteupTableWidget);
    VideTable(ui->ListeupTableWidget);
    ui->MontantlineEdit             ->setText("0,00");
    ui->TierscomboBox               ->clearEditText();
    ui->ComptesupComboBox           ->clearEditText();
    ui->CommissionlineEdit          ->setText("0,00");
...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
bool dlg_paiementdirect::isActeLocked(int idacte)
{
