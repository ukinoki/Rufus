...
dlg_paiementtiers::ResultEnregRecette dlg_paiementtiers::EnregistreRecette()
{
    ...
    if ((ui->EspecesradioButton->isChecked() || ui->VirementradioButton->isChecked() || ui->ChequeradioButton->isChecked())
         && m_mode == EnregistrePaiementTiers && ui->DetailupTableWidget->rowCount() > 0)
    {
        ...
        {
            double paye = 0;
            UpLineEdit* Line = qobject_cast<UpLineEdit*>(ui->DetailupTableWidget->cellWidget(i,8));
            if (Line)
...
void dlg_paiementtiers::CompleteDetailsTable(QTableWidget *TableOrigine, int Rangee, bool Coche)
{
    ...
    case VoirListePaiementsTiers:      // on reconstruit la liste des actes inclus dans le paiement en surbrillance
    {
        ...
        if (mp == CHQ)
        {
            ...
            ui->BanqueChequecomboBox->setCurrentText(rec->banquecheque());
        }
 ...
 . c'est un virement et il a été pointé sur le compte*/
    ...
    else
    {
        ...
        else
        {
            ...
            if (rec->banquecheque() == "")                                                                 //BanqueCheque
                m_lignerecetteamodifier << "null";
            else
                m_lignerecetteamodifier << "'" + Utils::correctquoteSQL(rec->banquecheque()) + "'";
...
{
    QString NettoieVerrousComptaActesRequete = "delete from " TBL_VERROUCOMPTAACTES " where " CP_POSEPAR_VERROUCOMPTA " = " + QString::number(currentuser()->id()) + " or PosePar is null";
