...
ui(new Ui::dlg_actesprecedents)
{
    ...
    QString style = "background-color:qlineargradient(x1: 0, y1: 0, x2: 1, y2: 1, stop: 0 #f6f7fa, stop: 1 rgb(200, 230, 250, 50));"
    ...
    if (m_avantdernieracte)
        restoreGeometry(proc->settings()->value(Position_Fiche "ActesPrec").toByteArray());
    else
        restoreGeometry(proc->settings()->value(Position_Fiche "AutreDossier").toByteArray());
...
{
    if (m_avantdernieracte)
        proc->settings()->setValue(Position_Fiche "ActesPrec", saveGeometry());
    else
        proc->settings()->setValue(Position_Fiche "AutreDossier", saveGeometry());
    event->accept();
}
...
void dlg_actesprecedents::ActesPrecsAfficheActe()
{
    ...
    if( acte->motif().size() || acte->texte().size() || acte->conclusion().size() )
    {
        ...
        if( acte->texte().size() )
        {
            ...
            textHTML += HTML_RETOURLIGNE "<td width=\"10\"></td><td width=\"450\">" + texte + "</td></p>";
        }
