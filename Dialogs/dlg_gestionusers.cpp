...
void dlg_gestionusers::Annulation()
{
    if (m_mode == Creer)
    {
        ...
        {
            int idcptasupprimer = Datas::I()->comptes->initListeComptesByIdUser(m_userencours->id()).firstKey();
            Datas::I()->comptes->SupprimeCompte(Datas::I()->comptes->getById(idcptasupprimer));
        }
        RemplirTableWidget(Datas::I()->users->userconnected()->id());
        ui->Principalframe->setEnabled(false);
        wdg_buttonframe->setEnabled(true);
