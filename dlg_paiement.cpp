    ...
    ui(new Ui::dlg_paiement)
{
    ...
    m_listeParents = proc->getListeParents(); // les colonnes -> iduser, userlogin, soignant, responsableactes, UserEnregHonoraires, idCompteEncaissHonoraires
    ...
    gDataUser                               = proc->getUserById(gidUserACrediter);
...
void dlg_paiement::Slot_Annul()
{
    ...
    else if (ModifPaiementEnCours)
    {
        ...
        OKBouton->setText(tr("Annuler la modification"));
    ...
    else
    {
           ...
           )
        {
            ...
            msgbox.setIcon(UpMessageBox::Warning);
            UpSmallButton *OKBouton = new UpSmallButton();
            OKBouton->setText(tr("Annuler l'écriture"));
            UpSmallButton *NoBouton = new UpSmallButton();
            msgbox.addButton(NoBouton, UpSmallButton::CANCELBUTTON);
...
{
    gDataUser = proc->getUserById(ui->UserscomboBox->currentData().toInt());
    if (gDataUser != nullptr)
    {
        gNomUser                            = gDataUser->getLogin();
        gidCompteBancaireParDefaut          = gDataUser->getIdCompteEncaissHonoraires();
    ...
    case EnregistrePaiementTiers:
    {
        if (VerifCoherencePaiement())
            ...
            case OK:
            {
                ...
                ModifLigneRecettePossible = true;
                Slot_RegleAffichageFiche();
                if (SupprimerBouton!=NULL) SupprimerBouton->setVisible(false);
                ui->AnnulupPushButton->setText("Annuler");
                NettoieVerrousCompta();
                break;
            }
...
int dlg_paiement::EnregistreRecette()
{
    ...
    if (gMode == EnregistrePaiementDirect)
    {
        if (ui->GratuitradioButton->isChecked())
        {
            for (int i = 0; i != ui->DetailupTableWidget->rowCount(); i++)
            {
                if (QLocale().toDouble(ui->DetailupTableWidget->item(i,5)->text()) > 0)
                {
                    ...
                    msgbox.setIcon(UpMessageBox::Warning);
                    UpSmallButton *OKBouton = new UpSmallButton();
                    OKBouton->setText(tr("Forcer l'enregistrement"));
                    UpSmallButton *NoBouton = new UpSmallButton();
        ...
        if (ui->CarteCreditradioButton->isChecked())
        {
            ModePaiement    = "'T'";
            TypeTiers       = "'CB'";
        }
        if (ui->EspecesradioButton->isChecked())
            ModePaiement = "'E'";
        if (ui->GratuitradioButton->isChecked())
            ModePaiement = "'G'";
        if (ui->ChequeradioButton->isChecked())
...
void dlg_paiement::CompleteDetailsTable(QTableWidget *TableSource, int Rangee, bool Coche)
{
    ...
    case EnregistrePaiementDirect:
    {
        if (Coche)
        {
            ...
            TableDetails->setRowHeight(i,fm.height()*1.1);
        ...
        else
        {
            ...
            if (!items.isEmpty())
            {
                ...
                for (int l = 0; l < NbLignesAVirer ;l++)
                {
                    ...
                    TableDetails->setRowHeight(l,fm.height()*1.1);
                }
            }
        }
    ...
    case VoirListeActes:
    {
        ...
        if (RetrouveTiersPayantQuery.size() > 0)
        {
         ...
         // => pas de break, on continue avec le code de VoirListePaiementsTiers
    }
...

{
    ...
    QString RetrouveRecetterequete = "SELECT idRecette, idUser, DatePaiement, DateEnregistrement, Montant, ModePaiement, TireurCheque,"
              "CompteVirement, BanqueCheque, TiersPayant, NomTiers, Commission, Monnaie, idRemise, EnAttente, EnregistrePar, TypeRecette, datediff(DateEnregistrement,NOW()) as Delai FROM "
            NOM_TABLE_RECETTES " WHERE idRecette = " + QString::number(idRecette);
    ...
    if (RadioAClicker != 0)
        RadioAClicker->setChecked(true);
    ui->ComptesupComboBox->clearEditText();
    if (RetrouveRecetteQuery.value(7).toString() != "0")                                                                    // CompteVirement
 ...
 . c'est un virement et il a été pointé sur le compte*/
    ...
    else
    {
        ...
        if (ChercheVirementCompteQuery.size() > 0)
        {
            ...
            if (ChercheVirementCompteQuery.value(9).toInt() == 1)
                Consolide = true;
        }
        if (!Consolide)
        {
        ...
        else
        {
            ...
            if (ChercheVirementCompteQuery.size() > 0)
            {
                ...
                LigneCompteAModifier << ChercheVirementCompteQuery.value(2).toString();                               //idDep
                LigneCompteAModifier << ChercheVirementCompteQuery.value(3).toString();                               //idRec
                if (ChercheVirementCompteQuery.value(4).toString().isEmpty())
                    LigneCompteAModifier << "null";
                else
                    LigneCompteAModifier << "'" + ChercheVirementCompteQuery.value(4).toDate().toString("yyyy-MM-dd") + "'";//LigneDate
                if (ChercheVirementCompteQuery.value(5).toString().isEmpty())
                    LigneCompteAModifier << "null";
                else
                    LigneCompteAModifier << "'" + proc->CorrigeApostrophe(ChercheVirementCompteQuery.value(5).toString()) + "'";//Lignelibelle
                LigneCompteAModifier << ChercheVirementCompteQuery.value(6).toString();                                     //LigneMontant
                LigneCompteAModifier << "'" + ChercheVirementCompteQuery.value(7).toString() + "'";                         //LigneDebiCredit
                if (ChercheVirementCompteQuery.value(8).toString().isEmpty())
                    LigneCompteAModifier << "null";
                else
                    LigneCompteAModifier << "'" + proc->CorrigeApostrophe(ChercheVirementCompteQuery.value(8).toString()) + "'";//LigneTypeOperation
                if (ChercheVirementCompteQuery.value(9).toInt() == 0)
                    LigneCompteAModifier << "null";
                else
                    LigneCompteAModifier << ChercheVirementCompteQuery.value(9).toString();                                 //Lignelibelle
                  ...
                  if (mp == "V")
                {
                    //Nettoyer et mettre en mémoire les commissions et les dépenses correspondantes éventuelles
                      ...
                      if (CommissionCompteQuery.size() > 0)
                      {
                          ...
                          if (CommissionCompteQuery.value(4).toString().isEmpty())
                          ...
                          if (CommissionCompteQuery.value(5).toString().isEmpty())
                              LigneCommissionCompteAModifier << "null";
                          else
                          ...
                          if (CommissionCompteQuery.value(8).toString().isEmpty())
                              LigneCommissionCompteAModifier << "null";
                          else
                      ...
                      QSqlQuery DepenseQuery (Depenserequete,db);
                      LigneDepenseAModifier.clear();
...
void dlg_paiement::PoseVerrouCompta(int ActeAVerrouiller)
{
    ...
    QSqlQuery verifVerrouQuery (verrourequete,db);
...
void dlg_paiement::RegleAffichageTypePaiementframe(bool VerifierEmetteur, bool AppeleParClicK)
{
    ...
    case EnregistrePaiementTiers:
    {
        ui->OKupPushButton      ->setEnabled(QLocale().toDouble(ui->MontantlineEdit->text()) > 0 || (Emetteur !=NULL) || ui->DetailupTableWidget->rowCount() > 0);
        ...
        for (int i = 0; i < TableQuery.size(); i++)
            {
                ...
                TableARemplir->setRowHeight(i,fm.height()*1.1);
...
bool dlg_paiement::VerifCoherencePaiement()
{
    ...
    while (Reponse) {
                ...
                && ui->BanqueChequecomboBox->findText(ui->BanqueChequecomboBox->currentText()) == -1)
        {
            ...
            OKBouton                    ->setText(tr("Enregistrer"));
            UpSmallButton *NoBouton     = new UpSmallButton();
            NoBouton                    ->setText(tr("Ne pas enregistrer"));
            UpSmallButton *OupsBouton   = new UpSmallButton();
            OupsBouton                  ->setText(tr("Annuler et revenir à la saisie"));
        ...
        if (ModifLigneRecettePossible && (ui->TierscomboBox->currentText() == "CB" && QLocale().toDouble(ui->CommissionlineEdit->text()) ==  0 && ui->VirementradioButton->isChecked()))
        {
