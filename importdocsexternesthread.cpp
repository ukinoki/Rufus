...
void ImportDocsExternesThread::RapatrieDocumentsThread(QStringList newdoclist)
{
    /*!
    -> newdoclist.at(0) = le titre de l'examen
    -> newdoclist.at(1) = le nom de l'appareil
    -> newdoclist.at(2) = le nom du fichier à importer
    */
    if (m_encours)
        return;
    if (newdoclist == QStringList())
    {
        m_encours = false;
        return;
    }
    ...
    if (!DefinitDossiersImagerie())
    {
        ...
        QString NomDirDoc = newdoclist.at(2);  // le dossier où sont exportés les documents d'un appareil donné
        if (NomDirDoc == "")
            ...
            /* Retrouver
                ...
                */
            // Titre du document------------------------------------------------------------------------------------------------------------------------------------------------
        QString Titredoc    = newdoclist.at(0);
        QString Typedoc     = Titredoc;
        QString SousTypeDoc = Titredoc;
        QString Appareil    = newdoclist.at(1);
