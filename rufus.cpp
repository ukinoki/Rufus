...
Rufus::Rufus(QWidget *parent) : QMainWindow(parent)
{
    ...
    qApp->setApplicationVersion("15-09-2026/1");
    ui = new Ui::Rufus;
    ui->setupUi(this);
    ...
    //! 5 mise à jour de la table des postes connectés
    PosteConnecte* postadmin = Datas::I()->postesconnectes->admin();
    /*! Nettoyage de la table TBL_USERSCONNECTES des erreurs éventuelles */
    //! suppression des utilisateurs disparus
    QString req = "delete from " TBL_USERSCONNECTES " where " CP_IDUSER_USRCONNECT " not in (select " CP_ID_USR " from " TBL_UTILISATEURS ")";
    db->StandardSQL(req);
    //! suppression des utilisateurs connectés sur ce poste
    req = "delete from " TBL_USERSCONNECTES " where " CP_MACADRESS_USRCONNECT " = '" + Utils::MACAdress() + "' and " CP_IDUSER_USRCONNECT " <> " + QString::number(currentuser()->id());
    db->StandardSQL(req);

    MAJPosteConnecte();
    if (postadmin == nullptr)
        VerifVerrouDossier();
    ...
    //! 7 - mettre en place le TcpSocket et/ou les timer
    ...
    //! 8 - Nettoyage des erreurs éventuelles de la salle d'attente
    ...
    //! 10 - libération des verrous de la compta
    req = " delete from " TBL_VERROUCOMPTAACTES " where " CP_POSEPAR_VERROUCOMPTA " = " + QString::number(currentuser()->id());
    db->StandardSQL(req);
    m_closeflag = false;
    setTitre();

    //!11 - reconstruction du QCompleter de la liste de cotations
    if (currentuser()->isSoignant())
    {
        if (ui->ActeCotationcomboBox->lineEdit()->completer() != nullptr)
        {
    ...
    //! 13 - Vérification de la messagerie
    ReconstruitListeMessages();

    //! 14 - Affichage des boutons bilan orthoptique et création intervention
    ...
    //! 15 - mise à jour du programmateur de sauvegarde
    if (db->ModeAccesDataBase() == Utils::Poste)
        proc->ParamAutoBackup();

    //! 16 - mise à jour du programmateur de l'effacement des fichiers images provisoires - abandonné parce qu'il continue à fonctionner même en cas de plantage du programme
     //if (db->ModeAccesDataBase() == Utils::Poste)
    ...
    //! 17 - choix mode (création dossier ou sélection de patient)
    ...
    //! 18 - suppression des anciens fichiers de log
    Logs::EpureLogs();

    //! 19 - Vérifie si une nouvelle version est disponible
    VerifLastVersion();
...
void Rufus::AfficheMotif(UpLabel *lbl)
{
    ...
    {
        if (id == it.key())
            if (const_cast<PatientEnCours*>(it.value()) != nullptr)
...
void Rufus::AfficheMenu(QMenu *menu)
{
    ...
    else if (menu == menuActe && menuActe !=nullptr)
    {
...
void Rufus::AppelPaiementDirect(Origin origin)
{
    ...
    else if (origin == BoutonPaiement)        // l'appel est fait par un clic sur le bouton enregistrepaiement
    {
        ...
        QString Msg;
        PatientEnCours *patcours = nullptr;
        for (auto it = Datas::I()->patientsencours->patientsencours()->constBegin(); it != Datas::I()->patientsencours->patientsencours()->constEnd(); ++it)
        {
...
void Rufus::AppelPaiementTiers()
{
    ...
    if(Dlg_PmtTiers->initOK())
    {
        ...
        connect(Dlg_PmtTiers, &QDialog::finished, this, [=, this]{
            ...
            {
                m_lignespaiements->initListeByPatient(currentpatient());
                if (currentacte() != nullptr && ui->tabDossier->isVisible())
                    AfficheActeCompta(currentacte());
            }
        });
    }
...
void Rufus::CreerBilanOrtho()
{
    ...
    QDate DateBl;
    RecalcCurrentDateTime();
    if (ui->Acteframe->isVisible() && currentacte() != nullptr)
    {
...
void Rufus::FiltreSalleDAttente()
{
    ...
    if (iduser==-1)
        ...
        for(int i=0; i<ui->SalleDAttenteupTableWidget->rowCount(); i++)
        {
            ...
            if (lbl == nullptr)               //! cellule absente (ligne en cours de reconstruction) → on ne touche pas à cette ligne
                continue;
...
/*! MEDICAL IMAGES  ============================================*/
    ...
    if (m_ok)
        ...
        {
            int iddoc           = listexportjpg.at(i).at(0).toInt();
            int idpat           = listexportjpg.at(i).at(1).toInt();
            QString sstypedoc   = listexportjpg.at(i).at(2).toString();
            QDate datetransfer  = listexportjpg.at(i).at(3).toDate();
            QByteArray ba       = listexportjpg.at(i).at(4).toByteArray();
            QString lienfichier = listexportjpg.at(i).at(5).toString();
            QString typedoc     = listexportjpg.at(i).at(6).toString();
            /*! si le lien vers le fichier est valide, on efface le champ jpg et on passe à la réponse suivante*/
            if (lienfichier != "")
            {
                QString CheminFichier = pathDirImagerie + NOM_DIR_IMAGES + lienfichier;
                if (QFile(CheminFichier).exists())
                {
                    db->StandardSQL("update " TBL_DOCSEXTERNES " set " CP_JPG_DOCSEXTERNES " = null where " CP_ID_DOCSEXTERNES " = " + QString::number(iddoc));
                    continue;
                }
            }
            ...
            {
                UpMessageBox::Watch(nullptr, tr("Impossible d'ouvrir le fichier") + " " + CheminOKTransfrDoc);
                continue;
            }
    ...
    if (m_ok)
        ...
        {
            int iddoc           = listexportpdf.at(i).at(0).toInt();
            int idpat           = listexportpdf.at(i).at(1).toInt();
            QString sstypedoc   = listexportpdf.at(i).at(2).toString();
            QDate datetransfer  = listexportpdf.at(i).at(3).toDate();
            QByteArray bapdf    = listexportpdf.at(i).at(4).toByteArray();
            QString lienfichier = listexportpdf.at(i).at(5).toString();
            QString typedoc     = listexportpdf.at(i).at(6).toString();
            ...
            if (lienfichier != "")
            {
                ...
                if (QFile(CheminFichier).exists())
                {
                    db->StandardSQL("update " TBL_DOCSEXTERNES " set " CP_PDF_DOCSEXTERNES " = null where " CP_ID_DOCSEXTERNES " = " + QString::number(iddoc));
                    continue;
                }
            }
...
/*! INVOICES  ============================================*/
    ...
    if (m_ok)
        ...
        {
            int idfacture           = listexportjpgfact.at(i).at(0).toInt();
            QDate datetransfer      = listexportjpgfact.at(i).at(1).toDate();
            QString lienfacture     = listexportjpgfact.at(i).at(2).toString();
            QString intitulefacture = listexportjpgfact.at(i).at(3).toString();
            QString typefacture     = (listexportjpgfact.at(i).at(4).toInt()==1? ECHEANCIER : FACTURE);
            int iddepense           = listexportjpgfact.at(i).at(5).toInt();
            QByteArray ba           = listexportjpgfact.at(i).at(6).toByteArray();

            /*! si le lien vers le fichier est valide, on efface le champ jpg et on passe à la réponse suivante*/
            if (lienfacture != "")
            {
                QString CheminFichier = pathDirImagerie + NOM_DIR_FACTURES + lienfacture;
                if (QFile(CheminFichier).exists())
                {
                    db->StandardSQL("update " TBL_FACTURES " set " CP_JPG_FACTURES " = null where " CP_ID_FACTURES " = " + QString::number(idfacture));
                    continue;
                }
            }
            ...
            */
            ...
            QString NomFileDoc = QString::number(idfacture) + "_"
                    + typefacture + "-"
                    + intitulefacture.replace("/",".") + "_"
                    + datetransfer.toString("yyyyMMdd");

            //! on recherche la dépense à l'origine de cette facture
            QList<QVariantList> Listedep;
            req = "select " CP_IDUSER_DEPENSES " from " TBL_DEPENSES " where " CP_IDFACTURE_DEPENSES " = " + QString::number(idfacture);
            ...
            int iduser = Listedep.at(0).at(0).toInt();
            User* user = Datas::I()->users->getById(iduser);
            if (user == nullptr)
            {
                /*! l'utilisateur référencé pour cette dépense n'existe pas, on détruit la facture et la dépense */
                db->SupprRecordFromTable(idfacture, CP_ID_FACTURES, TBL_FACTURES);
                db->SupprRecordFromTable(iddepense, CP_ID_DEPENSES, TBL_DEPENSES);
                continue;
            }
            ...
            /*! Si l'enregistrement et la compression du fichier ont réussi,
                ...
                *  enregistrement du lien vers le fichier sauvegardé
             */
            db->StandardSQL("update " TBL_FACTURES " set " CP_JPG_FACTURES " = null, " CP_LIENFICHIER_FACTURES " = '/" + user->login() + "/" + Utils::correctquoteSQL(NomFileDoc) + "'"
                            " where " CP_ID_FACTURES " = " + QString::number(idfacture));
            faits ++;
            int nsec = debut.secsTo(QTime::currentTime());
            int min = nsec/60;
            int hour = min/60;
    ...
    if (m_ok)
        ...
        {
            int idfacture           = listexportpdffact.at(i).at(0).toInt();
            QDate datetransfer      = listexportpdffact.at(i).at(1).toDate();
            QString lienfacture     = listexportpdffact.at(i).at(2).toString();
            QString intitulefacture = listexportpdffact.at(i).at(3).toString();
            QString typefacture     = (listexportpdffact.at(i).at(4).toInt()==1? ECHEANCIER : FACTURE);
            int iddepense           = listexportpdffact.at(i).at(5).toInt();
            QByteArray bapdf        = listexportpdffact.at(i).at(6).toByteArray();
            ...
            if (lienfacture != "")
            {
                ...
                if (QFile(CheminFichier).exists())
                {
                    db->StandardSQL("update " TBL_FACTURES " set " CP_PDF_FACTURES " = null where " CP_ID_FACTURES " = " + QString::number(idfacture));
                    continue;
                }
            }
            ...
            */
            ...
            int iduser = Listeusr.at(0).at(0).toInt();
            User* user = Datas::I()->users->getById(iduser);
            if (user == nullptr)
            {
                /*! l'utilisateur référencé pour cette dépense n'existe pas, on détruit la facture et la dépense */
            ...
            else
            {
                ...
                QString delreq = "delete from  " TBL_FACTURES " where " CP_ID_FACTURES " = " + QString::number(idfacture);
                //qDebug() << delreq;
                db->StandardSQL(delreq);
            ...
            /*! Si l'enregistrement et la compression du fichier ont réussi,
                ...
                *  enregistrement du lien vers le fichier sauvegardé
             */
            db->StandardSQL("update " TBL_FACTURES " set " CP_PDF_FACTURES " = null, " CP_LIENFICHIER_FACTURES " = '/" + user->login() + "/" + Utils::correctquoteSQL(NomFileDoc)  + "'"
                            " where " CP_ID_FACTURES " = " + QString::number(idfacture));
            faits ++;
            int nsec = debut.secsTo(QTime::currentTime());
            int min = nsec/60;
...
void Rufus::ImportNouveauDocExterne(AppareilImagerie *appareil)
{
    if (isPosteImport())
    {
        if (m_importdocsexternes == nullptr)
        {
...
void Rufus::ImprimeDossier(Patient *pat, bool quelepdf)
{
    ...
    if (currentpatient() != nullptr && pat == currentpatient())
...
void Rufus::ImprimeListActes(QList<Acte*> listeactes, bool toutledossier, bool queLePdf, QString nomdossier)
{
    ...
    if (testatcdtsgen != "" || testttgen != "" || testttoph != "" || testatcdtsophs != "" || cor != nullptr)
        Reponse += "<br>";
   ...
   if (queLePdf)
   {
       ...
       proc                ->Cree_pdffile(textcorps, textentete, textpied,
                               filename, nullptr, false, dirname);
...
void Rufus::ListeCorrespondants()
{
    if (Datas::I()->correspondants->correspondants()->size()==0)
    {
        ...
        dlg_identificationcorresp *Dlg_IdentCorresp = new dlg_identificationcorresp(dlg_identificationcorresp::Creation, onlydoctors, nullptr);
...
void Rufus::ListeManufacturers()
{
    if (Datas::I()->manufacturers->manufacturers()->size()==0)
    {
        ...
        dlg_identificationmanufacturer *Dlg_IdentManufacturer    = new dlg_identificationmanufacturer(dlg_identificationmanufacturer::Creation, nullptr);
...
void Rufus::MenuContextuelSalDat(UpLabel *labelClicked)
{
    if (currentuser()->isNeutre())
        return;
    int idpat (0);
    if (labelClicked == nullptr) return;
...
void Rufus::ChoixMenuContextuelSalDat(int idpat, QString choix)
{
    ...
    {
        PatientEnCours *patencours = nullptr;
        for (auto it = Datas::I()->patientsencours->patientsencours()->constBegin(); it != Datas::I()->patientsencours->patientsencours()->constEnd(); ++it)
        {
            if (idpat == it.key())
            {
...
QTabWidget* Rufus::Remplir_MsgTabWidget()
{
    ...
    if (listmessagesrecus.size() > 0)
    {
        ...
        for (int i=0; i<listmessagesrecus.size(); i++)
        {
            ...
            if (msg->idemetteur() > 0)
                txt += tr(" de ") + (Datas::I()->users->getById(msg->idemetteur()) != nullptr? Datas::I()->users->getById(msg->idemetteur())->login() : "null");
    ...
    if (listmessagesenvoyes.size() >0)
    {
        ...
        for (int i=0; i<listmessagesenvoyes.size(); i++)
        {
            ...
            if (msg->iddestinataire()>0)
                txt += tr(" pour ") + (Datas::I()->users->getById(msg->iddestinataire()) != nullptr? Datas::I()->users->getById(msg->iddestinataire())->login() : "null");
            Titredoc->setText(txt);
            titrelay->addWidget(Titredoc);
            UpCheckBox *Rdchk = new UpCheckBox();
            Rdchk->setChecked(msg->islu());
...
void Rufus::CleanSalleDAttente()
{
    ...
    */
    for (auto it = Datas::I()->postesconnectes->postesconnectes()->constBegin(); it != Datas::I()->postesconnectes->postesconnectes()->constEnd(); ++it)
    {
        ...
        if (post != nullptr)
            if (Datas::I()->users->getById(post->iduser()) != nullptr)
...
void Rufus::VerifVerrouDossier()
{
    ...
    foreach(PosteConnecte* post, *Datas::I()->postesconnectes->postesconnectes())
    {
        ...
        if (tempsecouledepuisactualisation > 240)
        {
            ...
            qDebug() << "user = " << (Datas::I()->users->getById(post->iduser()) != nullptr? Datas::I()->users->getById(post->iduser())->login() : tr("inconnu"));
            ...
            for (int i=0; i<xml.childNodes().size(); i++)
            {
                ...
                for (int j=0; j<system.childNodes().size(); j++)
                {
                    ...
                    else if (child.tagName() == "Comment")
                    {
                        ...
                        //! <ES>, <PT>, <BR>, <IT>, <RO>, <CA>. On affiche celle qui correspond à la langue du poste
                        ...
                        for (int k=0; k<child.childNodes().size(); k++)
                        {
                            ...
                            if (tag=="FR" || tag=="EN" || tag=="ES" || tag=="PT" || tag=="BR" || tag=="IT" || tag=="RO" || tag=="CA")
                            {
                                hassections = true;
...
bool Rufus::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (event->type() == QEvent::FocusOut )
    {
        ...
        else
        {
            ...
            if (objUpLine != nullptr && obj != wdg_MGlineEdit && obj != wdg_autresCorresp1LineEdit && obj != wdg_autresCorresp2LineEdit)
            {
    ...
    // RETURN _________________________________________________________________________________________________________________________________________________________________
        ...
        if ((keyEvent->key()==Qt::Key_Return  && keyEvent->modifiers() == Qt::NoModifier) || keyEvent->key() == Qt::Key_Enter)
        {
            ...
            if (qobject_cast<UpTextEdit *>(obj) != nullptr)
            {
                UpTextEdit* objUpText = qobject_cast<UpTextEdit*>(obj);
...
void Rufus::AfficheActe(Acte* acte)
{
    ...
    if (usr != nullptr)
    {
        ...
        superviseurlogin = usr->login();
        if (Datas::I()->cotations->userparent() != usr)
            Datas::I()->cotations->loadUserCotations(usr);
        ReconstruitComboCotations(usr);
    }
    ...
    ui->SitelineEdit->setText(Datas::I()->sites->getById(acte->idsite()) != nullptr? Datas::I()->sites->getById(acte->idsite())->nom() : "");
...
//  Afficher les éléments de la tables Patients
     ...
     */
    if (proc->PortRefracteur()!=nullptr || proc->LANRefracteur())
...
void Rufus::OuvrirDossier(Patient *pat, int idacte,  QWidget *parent)  // appelée depuis la tablist ou la salle d'attente - vérifie qu'un dossier n'est pas verrouillé avant de l'afficher
{
    ...
    {
        if (ui->tabWidget->indexOf(ui->tabDossier) > 0 && currentpatient() != nullptr)
        {
...
bool Rufus::IdentificationPatient(dlg_identificationpatient::Mode mode, Patient *pat)
{
    ...
    {
        if (mode == dlg_identificationpatient::Copie && Dlg_IdentPatient->currentpatient() == nullptr)  // il n'y a pas eu de copie parce que le dossssier existait déjà
        {
            QString Sexe ("");
    ...
    if (unpatientaetecreeoumodifie && Dlg_IdentPatient->currentpatient() != nullptr && m_utiliseTCP)
...
{
    int compta = m_parametres->compta();
...
void    Rufus::ReconstruitComboCotations(User *usr)
{
...
//    if (proc->FicheRefractionOuverte())
    if (findChildren<dlg_refraction*>().size()>0)
        return;
    if (currentpatient() == nullptr || currentacte() == nullptr)
        return;
 ...
 */
    ...
    if (proc->settings()->value("Param_Poste/Refracteur").toString()=="NIDEK Glasspop")
        while (itref.hasPrevious()) {  // si on a un autoref frais du jour on le prend
            itref.previous();
            if (Datas::I()->actes->getById(itref.value()->idacte()) != nullptr)
...
bool Rufus::Remplir_ListePatients_TableView()
{
    ...
    if (m_listepatientsmodel != nullptr)
    ...
    if (m_listepatientsproxymodel == nullptr)
...
/*! ---------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if (m_listesuperviseursmodel != nullptr)
...
/*! ---------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if (m_listesuperviseursaccueilmodel != nullptr)
    ...
    foreach (PatientEnCours *patencours, listpatvus)
    {
        ...
        if (actapayer == nullptr)
            continue;

        QString superviseurlogin    = (Datas::I()->users->getById(patencours->idusersuperviseur()) != nullptr? Datas::I()->users->getById(patencours->idusersuperviseur())->login() : "");
    ...
    if (m_utiliseTCP)
    {
        for (auto it = m_listesockets.begin(); it != m_listesockets.end(); ++it)
        {
            ...
            if (it == m_listesockets.begin())
            {
                ...
                if (post != nullptr) //!>* chaque item contient adresseIP, adresseMac, LoaclhostName(), idUser puis  TCPMSG_ListeSockets
                {
                    m_resumeStatut += post->ipadress() + " - "
                            + post->macadress() + " - "
                            + post->nomposte() + " --- "
                            + (Datas::I()->users->getById(post->iduser()) != nullptr? Datas::I()->users->getById(post->iduser())->login() : "") ;
                }
                else
...
void Rufus::TraiteTCPMessage(QString msg)
{
    ...
    else if (msg.contains(TCPMSG_MAJDocsExternes))
    {
        ...
        if (currentuser()->isSoignant() && currentpatient() != nullptr)
