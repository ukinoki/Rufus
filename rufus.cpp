...
void Rufus::AfficheMotif(UpLabel *lbl)
{
    ...
    QMap<QString,QVariant> mapage = Utils::CalculAge(rsgnmt["ddnpat"].toDate(), QDate::currentDate());
    QString Msg("");
...
void Rufus::AfficheToolTip(Patient *pat)
{
    ...
    {
        QMap<QString,QVariant> mapage = Utils::CalculAge(pat->datedenaissance(), QDate::currentDate());
        Msg += mapage["toString"].toString();
    }
    if (pat->ville() != "")
    {
        ...
        for (int i=0; i<listimages.size(); i++)
        {
            ...
            QString filedest = pat->nom() + " " + pat->prenom() + " - " + docmt->typedoc() + " " + docmt->soustypedoc() + " " + QString::number(i);
            {
                if (db->ModeAccesDataBase() != Utils::Distant)
                {
                    ...
                    origin.copy(nomdossier + "/" + filedest + "." + QFileInfo(origin).suffix().toLower());
                }
                else
                {
                    QByteArray ba;
...
void    Rufus::ImprimeDocument(Patient *pat)
{
    ...
    if (Dlg_Imprs->exec() == QDialog::Accepted)
    {
        ...
        foreach (mapdoc, listdocs)
        {
            ...
            aa                          = proc->Imprimer_Document(this, pat, userEntete, Titre, TxtDocument, DateDoc, Prescription, ALD, AvecPrevisu, AvecDupli, AvecChoixImprimante, Administratif);
            if (!aa)
                break;
