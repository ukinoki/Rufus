...
MySQLInstaller::RecupererMotDePasseMySQL(QWidget *parent, bool avecSecoursEtCompteMySQL)
{
    ...
    auto connexionValide = [parent](const QString& login, const QString& mdp) {
        ...
        const bool cnx  = m.tryConnectAs(login, mdp);
        const bool priv = cnx && m.checkPrivileges(manquants);
        const QString err = priv ? DataBase::I()->connectToDataBase(DB_RUFUS, login, mdp) : QString("(non tenté)");
        qDebug() << "connexionValide" << login << "tryConnectAs=" << cnx
                 << "checkPrivileges=" << priv << "manquants=" << manquants
                 << "connectToDataBase=" << err;
        return cnx && priv && err.isEmpty();
    };
        ...
        if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            ts << "$ErrorActionPreference = 'Stop'\r\n"
               ...
               << "  Copy-Item -Path (Join-Path (Join-Path $dest '" << innerDir << "') '*') "
                  "-Destination $base -Recurse -Force\r\n"
               ...
               << "  if ($qui) { \"VERROU $qui\" | Out-File -FilePath $log -Encoding utf8 -Force }\r\n"
               << "  else { \"$($_.Exception.Message) [ligne $($_.InvocationInfo.ScriptLineNumber) : $($_.InvocationInfo.Line.Trim())]\" | Out-File -FilePath $log -Encoding utf8 -Force }\r\n"
               << "}\r\n";
            f.close();
        }
    }
