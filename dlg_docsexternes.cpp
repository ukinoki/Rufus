...
void dlg_docsexternes::ImprimeDoc()
{
    ...
    if (idimpr != "")
    {
                 ...
                 || msgbox.clickedButton() == ImpBouton)
        {
            ...
            Entete.replace(QRegExp("<!--date-->[éêëèÉÈÊËàâÂÀîïÏÎôöÔÖùüûÙçÇ'a-zA-ZŒœ0-9°, -]*<!--date-->"), "<!--date-->" + gDataUser->getSite()->ville() + tr(" le, ") + QDate::currentDate().toString(tr("d MMMM yyyy")) + "<!--date-->");
            //création du pied
