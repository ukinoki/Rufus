...
void Procedures::setDirSSLKeys(QWidget *parent)
{
    ...
    if (dirkeys == "")
    {
        ...
        while (!a)
        {
            urlkeys = QFileDialog::getExistingDirectory(parent,
...
Procedures::typeEnvoi Procedures::QuestionMailPdfOrPrint(QWidget *parent, typeEnvoi typ, int idsite)
{
    ...
    dlg                     ->AjouteWidgetLayButtons(Utils::BoutonAide(
                                ...
                                tr("Que puis-je faire dans cette fiche ?")), false);
...
bool Procedures::Print(QList<QImage> listimage, QWidget *parent)
{
    ...
    {
        QPrintDialog *dialog = new QPrintDialog(m_printer, parent);
        if (dialog->exec() == QDialog::Accepted)
        {
        ...
        if (db->ModeAccesDataBase() != Utils::Poste || MySQLInstaller::socleLocalConforme())
        {
            forever
            {
                const MySQLInstaller::IssueMdp issue =
                    MySQLInstaller::RecupererMotDePasseMySQL(parent, QString(), QString(), db->ModeAccesDataBase() == Utils::Poste);
...
bool Procedures::IdentificationUser(QWidget *parent)
{
    ...
    bool ok = false;
    dlg_identificationuser *dlg_IdentUser   = new dlg_identificationuser(parent);
    ...
    if (ok)
    {
        ...
        //! contribuer via www.rufusvision.org.
        {
            ...
            if (lang != "FR" && !m_settings->value(cleAvis, false).toBool())
            {
                ...
                {
                    titre = "O notă despre această traducere";
                    texte = "Această versiune non-franceză a Rufus a fost tradusă cu ajutorul inteligenței "
