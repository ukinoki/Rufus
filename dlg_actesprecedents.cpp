...
void dlg_actesprecedents::Actualise()
{
    ...
    ui->ScrollBar->setMinimum(1);
    ui->ScrollBar->setMaximum(nbActes);
    ui->ScrollBar->setSingleStep(1);
    ui->ScrollBar->setValue(nbActes-1);
    ActesPrecsAfficheActe(ChercheActeAAfficher());
    connect(ui->ScrollBar, &QScrollBar::valueChanged, [=] {ActesPrecsAfficheActe(listactes.at(ui->ScrollBar->value()+1));});
}
...
void dlg_actesprecedents::ActesPrecsAfficheActe(int idActeAAfficher)
{
    ...
    else
    {
        ...
        {
            UpTextEdit textprov;
            QString Reponse = "<p style = \"margin-top:0px; margin-bottom:0px;\"><td width=\"120\"><font color = \"" + proc->CouleurTitres + "\" ><u><b>" + ActesPrecsQuery.value(3).toDate().toString(tr("d MMMM yyyy"))
                    + "</b></u></font></td>"
                    + "<td width=\"80\">"
            ...
            if (ActesPrecsQuery.value(4).toString() != "")
            {
                ...
                if (proc->MajusculePremiereLettre(textprov.toPlainText())!="")
                {
                    ...
                    Reponse += "<p style = \"margin-top:0px; margin-bottom:0px;\"><td width=\"10\"></td><td width=\"450\">" + texte + "</td></p>";
                }
            }
            ...
            ui->TexteupTextEdit->setText(Reponse);
